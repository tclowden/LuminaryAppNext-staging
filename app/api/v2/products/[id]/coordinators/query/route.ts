...
async function queryCoordinatorsOnProduct(request: NextRequest, options: any) {
   try {
      ...
      const results = await db.coordinatorsOnProducts.unscoped().findAll({
         ...filterData,
         where: {
            productId: id,
            ...filterData?.where,
...
export { queryCoordinatorsOnProduct as POST };
