...
async function queryTasksOnProduct(request: NextRequest, options: any) {
   try {
      ...
      const results = await db.tasksOnProducts.unscoped().findAll({
         ...filterData,
         where: {
            productId: id,
            ...filterData?.where,
...
export { queryTasksOnProduct as POST };
