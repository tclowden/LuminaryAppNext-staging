...
const NotesModal = ({ modalOpen, setModalOpen, initValues }: Props) => {
   const { lead } = useAppSelector(selectPageContext);
   ...
   const handleSaveNote = (e: any, updatedData: any) => {
      setIsSaving(true);
      const fullNoteOnLead = lead?.notesOnLead?.find((note: any) => note?.id === updatedData?.id);

      let dataToSave = null;
      if (fullNoteOnLead) dataToSave = { ...fullNoteOnLead, ...updatedData };
      ...
      console.log('dataToSave updating:', dataToSave);
