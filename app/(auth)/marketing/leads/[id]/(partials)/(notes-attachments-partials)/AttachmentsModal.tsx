...
const AttachmentsModal = ({ modalOpen, setModalOpen, initValues }: Props) => {
   const { lead, attachmentTypes } = useAppSelector(selectPageContext);
   const user = useAppSelector(selectUser);
   const dispatch = useAppDispatch();
   const [isSaving, setIsSaving] = useState<boolean>(false);
   const [validationSchema, setValidationSchema] = useState<YupSchemaObject<any>>();

   const handleSaveAttachment = (e: any, updatedData: any) => {
      setIsSaving(true);
      ...
      if (copy?.id) {
         ...
         if (!dataToSave?.leadId) dataToSave['leadId'] = lead?.id;
         ...
         dataToSave.append('filePath', `leads/${lead?.id}/attachments`);
         dataToSave.append('leadId', lead?.id);
      ...
      fetch(url, {
         ...
         .then(async (res: any) => {
            ...
            const tempAttachmentsOnLead = await getAttachments(user?.token || '', lead?.id);
            dispatch(setPageContext({ lead: { ...lead, attachmentsOnLead: tempAttachmentsOnLead } }));
         ...
         .catch((err: any) => {
            console.log('err saving attachment...:', err);
            setIsSaving(false);
   ...
   useEffect(() => {
      ...
      if (modalOpen && initValuesHasLength) {
         const installAgreementAttachmentType = attachmentTypes.find((at: any) => at?.name === 'Lead Record');
         ...
         const workOrderAttachmentType = attachmentTypes.find((at: any) => at?.name === 'Lead Record');
