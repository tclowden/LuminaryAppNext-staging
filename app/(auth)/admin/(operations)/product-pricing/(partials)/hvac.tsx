...
const HvacTable = ({ values, handleChange, handleBlur }: props) => {
   useEffect(() => {
   ...
   const handleInputChange = (id: any, field: any, value: any) => {
      ...
      const updatedhvacs = values.hvacs.map((hvac: any) => {
         const copyhvac = hvac;
         if (copyhvac.id === id) {
            let updatedhvac = copyhvac;
            updatedhvac[field] = value;
            return updatedhvac;
         }
         return copyhvac;
      });

      handleChange({ target: { type: 'text', name: 'hvacs', value: updatedhvacs } });
   };

   const handleDeleteAction = ({ actionKey, item, newData }: any) => {
      const updatedhvacs = [...values.hvacs].filter((hvac) => hvac?.id !== item?.id);
      handleChange({ target: { type: 'text', name: 'hvacs', value: updatedhvacs } });
   };

   const handleAddOption = () => {
      const randomId = Math.random().toString().slice(2, 7);
      if (values?.hvacs == undefined) {
         values.hvacs = [];
      }
      const tempHvacs = [...values?.hvacs, { id: randomId, name: '', price: '', actionsConfig: { delete: true } }];
