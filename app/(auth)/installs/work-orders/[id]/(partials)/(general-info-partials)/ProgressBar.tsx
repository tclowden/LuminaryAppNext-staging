...
import { IconColors } from '../../../../../../../common/components/Icon';
...
const ProgressBar = ({
   ...
   const progressBarColorClass = backgroundColorObject[strokeColor as keyof object];

   let indicatorColorClass = '';
   const [color, value] = strokeColor.split(':');
   if (!value) {
      indicatorColorClass = backgroundColorObject[`${color}:600` as keyof object];
   } else {
      indicatorColorClass = backgroundColorObject[`${color}:${+value + 100}` as keyof object];
   }
   return (
      <div
