...
const InProgress = ({ inProgress, updateProposalQueue }: Props) => {
   ...
   useEffect(() => {
      const dataToStore: any = inProgress
         .map((u: any, i: number) => ({
            ...u,
      ...
      <div>
         <Table
            actions={[
               ...
               { icon: 'Duplicate', actionKey: 'complete', toolTip: 'Complete', callback: handleActionClick },
            ]}
            columns={columns}
            data={data}
            onCellEvent={({ event }) => {
         ...
         <ConfirmModal
            ...
            handleOnConfirm={() => complete()}
            confirmationText={'Mark proposal as complete?'}
         />
      </div>
   );
};
